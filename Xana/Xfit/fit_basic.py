...
def lnlike(pars, x, func, data=None, eps=None):
    ...
    return -0.5*(np.sum(residual(pars, x, func, data)**2*inv_sigma2 - np.log(inv_sigma2/(2*np.pi))))
...
def fit_basic( x, y, dy=None, model='line', init={}, fix=None, method='leastsq',
    ...
    if emcee:
        mi = lmfit.minimize(residual, pars, args=(x, model), kws={'data':y, 'eps':wgt},
                            method='nelder', nan_policy='omit')
        # mi.params.add('f', value=1, min=0.001, max=2)
        mini = lmfit.Minimizer(residual, mi.params, fcn_args=(x, model), fcn_kws={'data':y, 'eps':wgt})
        out = mini.emcee(burn=300, steps=1000, thin=20, params=mi.params, is_weighted=is_weighted)
