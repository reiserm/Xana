...
def fitg2( t, cf, err=None, mode='semilogx', modes=1, init={}, fix=None, dofit=1,
    ...
    def res(pars, x, data=None, eps=None):
        ...
        for i in range(modes):
            if i == 0:
                model = g2(x, t=v['t{}'.format(i)], b=v['b{}'.format(i)],
                           g=v['g{}'.format(i)], a=v['a'])
            else:
                model += g2(x, t=v['t{}'.format(i)], b=v['b{}'.format(i)],
                            g=v['g{}'.format(i)], a=0)
        if eps is not None:
    ...
    if doplot:
        ...
        if dofit:
            for i in range(modes):
                if i == 0:
                    g2f = g2(xf, t=out.params['t{}'.format(i)].value,
                    ...
                    g2f += g2(xf, t=out.params['t{}'.format(i)].value,
                              b=out.params['b{}'.format(i)].value,
                              g=out.params['g{}'.format(i)].value, a=0)
        ...
        if 'legd' in doplot:
            if qv is not None:
                labstr_data = r'$\mathsf{{q}} = {:.3f}\,\mathsf{{nm}}^{{-1}}$'.format(qv)
        ...
        if 'data' in doplot:
            ...
            if err is not None:
                pl.append(ax.errorbar(t, cf, yerr=err, linestyle='', marker=marker,
                                      label=labstr_data, alpha=alpha, markersize=markersize))
            else:
                pl.append(ax.plot(t, cf, marker, label=labstr_data, alpha=alpha,
