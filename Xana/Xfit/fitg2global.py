...
class G2:
    ...
    def fit(self, mode='semilogx', nmodes=1, fitglobal=[], init={}, fix={}, errthr=1e-4, **kwargs):
        '''
        ...
        if bool(self.fitglobal):
            out = lmfit.minimize(self._residuals, self._lmpars,
    ...
    def _residuals(self, pars, x, data=None, eps=None):
    ...
    def _init_parameters(self, init, fix):
        ...
        # setting parameter contraints
        if self.nmodes > 1:
            beta_constraint = 'a+beta-1-'+'-'.join(['b{}'.format(x) for x in range(1,self.nmodes)])
        ...
        # modifying pars for global fitting
        pv = list(pars.values())
        ...
        regexp0 = re.compile('beta_\d{1}')
        regexp1 = re.compile('b\d{1}')
        for p in list(pars.keys()):
            if '_' in p and p.startswith('b') and not p.startswith('be'):
                ...
                if bool(texp) and self.nmodes > 1:
                    texp = regexp0.sub('beta_%s' % pt, texp)
                    tmp  = regexp1.findall(texp)[0]
                    texp = regexp1.sub(tmp+'_%s' % pt, texp)
