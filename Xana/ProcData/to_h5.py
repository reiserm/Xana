...
def h5_dump(fname, nframes, dataQ, dtype=np.dtype('uint16')):
    ...
    with h5.File(fname, 'w', driver='stdio') as f:
        ...
        while t0 < nframes:
            ...
            if t0 == 0:
                f.require_dataset('data', (nframes,*chunk[1].shape[1:]), dtype=dtype, compression='gzip')
...
def to_h5(obj, sid, filename, nprocs=4, chunk_size=100, dtype=np.dtype('uint16'), **kwargs):

    print('\n\n#### Writing Data to HDF5 File ####\nSeries: {} in folder {}\n'.format(sid, obj.meta.loc[sid, 'datdir']))

    read_data_opt = {'dtype':dtype,
                     'nprocs':1,
                     'verbose':False,
    }
    ...
    chunks = [np.arange(i*chunk_size,min((i+1)*chunk_size,fmax))
    ...
    for ip in range(nprocs):
        procs.append(mp.Process(target=obj.get_series,
