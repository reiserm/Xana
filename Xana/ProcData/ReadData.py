...
class dataset:
    ...
    def calc_mean(self, weighted=False):
        ...
        m, v = np.ma.average(self.dstream, axis=0, weights=1/self.variance,
                             returned=True)
        v = 1/v
        ...
        if self.output == 'original':
            ...
            if '2d' in self.output:
                if self.cond_rearrange_tiles():
                    self.dstream = rearrange_tiles(
                        self.dstream, self.arrange_tiles)
                ...
                if 'sec' in self.output:
                    if self.cond_section():
                        ...
                        self.dstream = self.dstream[..., qsec[0][0]:qsec[1][0]+1,
                                                    qsec[0][1]:qsec[1][1]+1]
                        if self.variance is not None:
                            self.variance = self.variance[..., qsec[0][0]:qsec[1][0]+1,
                                                          qsec[0][1]:qsec[1][1]+1]
    ...
    def cond_section(self):
        ...
        qsec = self.qsec
        if qsec is not None:
            if self.dstream.shape[-2:] != (qsec[1][0]-qsec[0][0]+1, qsec[1][1]-qsec[0][1]+1):
                return True
            else:
                return False
        else:
...
class sglimgfmt(dataset):
    ...
    def __init__(self, masterfile, datafiles, opt, use_chunks=True):
        ...
        super().__init__(opt)
        self.masterfile = masterfile
        if masterfile.endswith('edf') or masterfile.endswith('edf.gz'):
            self.get_image = edf.loadedf
        elif masterfile.endswith('cbf'):
    ...
    def start_reading_data(self):
        ...
        for ip in range(self.nprocs):
            ...
            self.procs.append(mp.Process(target=self.load_proc,
                                         args=(
                                             self.in_queue[ip], self.out_queue[ip],),
                                         kwargs={'qsec': self.qsec,
                                                 'dtype': self.dtype}))
            self.procs[ip].start()
...
# ----------------------
def read_data(datafiles, detector=None, last=None, first=None, step=[1, 1, 1], qroi=None,
              qsec=None, verbose=True, method='full', output='2dsection', chunk_size=256,
