...
def plot_poissongamma(x, y, p, M, cmap='tab20', ind_var='kb', confint=None,
                     q=None, ax=None, xlim=None, log='xy', **kwargs):
    if ax is None:
        ax = plt.gca()
    ...
    for i,pi in enumerate(p):
        print(pi, M)
        # plot data 
        ax.errorbar(1/x, y[i], yerr=None, marker='o', linestyle='',
        ...
        print(pbf)
        ax.plot(1/xf, pbf, '-')
        ax.annotate(r'$\beta={0:.2g}\pm{1:.2g}$'.format(1/M[0][0], 1/M[0][0]**2*M[0][1]), xy=(.8,.1),
