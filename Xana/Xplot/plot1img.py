...
def getimg(run, imgn, fnames, dim, ffmask=0, qroi=0, rgn=0):
    img = getNXimagedata(fnames[run], first=(imgn,0,0), last=(imgn+1,dim[1],dim[0]))
    img = np.squeeze(img)
    if rgn:
...
def browse_images(nf=0, run=0, fnames=0, dim=0, exp=0, gp=0, im=0, txt=0, fig=0, ffmask=0, qroi=0, rgn=0):
