...
def staticcontrast(obj, nq=None, ax=None, data='original', cmap='jet',
    ...
    for j, (cfi, dcfi) in enumerate(corrFunc):
        ...
        for i,qi in enumerate(obj.nq):
            ...
            rates[j, i, 1, :] = np.ma.average( cfi[t_ind+1,qi+1], weights=1/dcfi[t_ind+1,qi+1]**2, returned=1)
            rates[j, i, 1, 1] = np.sqrt(1/rates[j, i, 1, 1])
            ax[0].errorbar(ti, cfi[1:,qi+1], dcfi[1:,qi+1], label='{:.2e}nm-1'.format(obj.Xana.setup['qv'][qi]),
                        color=obj.colors[ci], marker=obj.markers[j%len(obj.markers)], markersize=2)
            ax[0].plot(tf, np.ones_like(tf)*rates[j, i, 1, 0], color=obj.colors[ci], )

            ax[1].errorbar(rates[j, i,0,1], rates[j, i,1,0], rates[j, i,1,1], color=obj.colors[ci],
                               label=cf_id, marker=obj.markers[j%len(obj.markers)], markersize=markersize)
            if trace:
                axtr.errorbar(rates[j, i,0,1], tr[j,qi,0], tr[j,qi,1], fmt='s', color=obj.colors[ci])
            ax[2].errorbar(tr[j,i,0], rates[j, i,1,0], xerr=tr[j,i,1], yerr=rates[j, i,1,1],
                                   color=obj.colors[ci], marker=obj.markers[j%len(obj.markers)],
