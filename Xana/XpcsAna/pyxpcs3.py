...
def calc_twotime_cf(ttdata, tt_max_images=5000):
    ...
    for ibin in range(nbins):
        ...
        if nf > tt_max_images:
            ...
            print('Reducing two-time correlation data from '
                  '{} to {} images by rebinning.'.format(nf, tt_max_images))
            data = np.mean(
                data[:nfnew].reshape(ttchunk, -1, lind, order='F'), 0)
...
def pyxpcs(data, qroi=None, dt=1., qv=None, saxs=None, mask=None, ctr=(0, 0),
           twotime_par=None, qsec=(0, 0), norm='symmetric_whole', nprocs=1,
           time_spacing=None, verbose=True, chn=16, tt_max_images=5000,
           use_multitau='auto', rebin_g2='auto'):
    ...
    if use_multitau:
        ...
        if verbose:
            print('Number of registers is {} with {} total '
    ...
    if use_mp:
        ...
        for i in range(nprocs):
            ...
            pcorr.append(Process(target=mp_corr,
                                 args=(nf-1, chn, srch, rcr, lind[q_beg:q_end],
                                       q_end-q_beg),
    ...
    while t0 < nf - 1:
        ...
        if chunk_diff != 1:
            raise IOError('Chunks have been read in wrong order: chunk index '
        ...
        for jj, (i, j) in enumerate(zip(q_sec[:-1], q_sec[1:])):
            ...
            for qi in range(i, j):
                ...
                if norm == 'symmetric':
                    ...
                    data_loop = (tmp_mat-tmp_mat.mean(-1)[:, None]) \
