    ...
    def correlator2(reg, matrx):
       ...
       # print('kreg: ', kreg)
        ...
        if  (chn2+1) < kreg <= chn:
            ...
            nk[inb:ine] += 1
            for cindex in xnq:
                x = np.dot(matrx[cindex],datreg[reg][cindex][:,chn2:kreg-1]) / lind[cindex]
                #print('x: ', x)
                running_mean(corf[cindex,inb:ine], dcorf[cindex,inb:ine], x, nk[inb:ine])
                datreg[reg][cindex] = np.concatenate(
                    (matrx[cindex][:,None], datreg[reg][cindex][:,:chn-1]), axis=1)
            verbose1(reg)
            ...
            nk[inb:ine] += 1
            for cindex in xnq:
                x = np.dot(matrx[cindex], datreg[reg][cindex][:,chn2:chn]) / lind[cindex]
                #print('x: ', x)
                running_mean(corf[cindex,inb:ine], dcorf[cindex,inb:ine], x, nk[inb:ine])
                datreg[reg][cindex] = np.concatenate(
                    (matrx[cindex][:,None], datreg[reg][cindex][:,:chn-1]), axis=1)
