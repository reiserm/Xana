...
def xsvs(obj, series_id, first=0, last=None, filename="", method='full', nbins=15,
         handle_existing='next', nprocs=4, nread_procs=4, chunk_size=100, dark=None,
         load_verbose=False, dtype=np.float32, **kwargs):

    #MyManager.register("PriorityQueue", PriorityQueue)  # Register a shared PriorityQueue

    for sid in series_id:
        print('\n\n#### Starting XSVS Analysis ####\nSeries: {} in folder {}\n'.format(sid, obj.xdata.datdir))
        ...
        chunks = [np.arange(first+i*chunk_size,min([min(first+(i+1)*chunk_size,last),fmax]))
...
#        m = Manager()
        ...
        for ip in range(nread_procs):
            procs.append(mp.Process(target=obj.xdata.get_series,
                                    args=(sid,), kwargs=read_data_opt))
            procs[ip].start()

        probd = pyxsvs(xpcs_opt, obj.setup['qroi'], method=method, nbins=nbins, t_e=t_e,
                       qv=obj.setup['qv'], nprocs=nprocs, qsec=obj.setup['qsec'][0])
        ...
        savfile = save_result(probd, 'xsvs', obj.savdir, f, handle_existing=handle_existing)
